    ...
    const handleMediaUpload=(e)=>{
        ...
        if(file){
            if(file.type.startsWith('video')){
                if(file.size>MAX_VIDEO_DURATION*1024*1024){
    ...
    <div className='fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4'>
        <div className='w-full max-w-md'>
                ...
                }
                {
                    mode==='media' && previewUrl &&(
                        media?.type.startsWith('image/') ?(
                            ...
                            <video src={previewUrl} alt="" className='object-contain max-h-full' controls/>
                        )
                    )
                }
            ...
            <div className='flex gap-2 mt-4'>
                <button className={`flex-1 flex items-center justify-center gap-2  p-2 rounded cursor-pointer ${mode==='text'?"bg-white text-black":"bg-zinc-700"}`} onClick={()=>{setMode('text');setMedia(null);setPreviewUrl(null)}}>
                    <TextIcon size={18}/>Text
                </button>
                <label className={`flex-1 flex items-center justify-center gap-2 p-2 rounded cursor-pointer ${mode==='media'?"bg-white text-black":"bg-zinc-700"}`}>
